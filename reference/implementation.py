...
def growDecisionTreeFrom(rows, evaluationFunction=entropy):
	...
	for col in range(0, columnCount):
		columnValues = [row[col] for row in rows] # traverse row by row through diff column
	...
	else: # a result was found, the unique counts will be a dictionary with one value - leaf node which will be stored at tree.results
...
if __name__ == '__main__':
	...
	if example == 1:
		...
		print(classify([6.0, 2.2, 4.9, 1.7], decisionTree)) # dataMissing=False is the default setting
		# print(classify([None, None, None, 1.5], decisionTree, dataMissing=True)) # no longer unique

	elif example == 3:
		df_adult_income = pd.read_excel("adult_income.xlsx", sheet_name=0)
		# standard_scalar = StandardScaler()
		# df_adult_income.iloc[:, [2, 10, 11]] = standard_scalar.fit_transform(df_adult_income.iloc[:, [2, 10, 11]])
		label_encoder = preprocessing.LabelEncoder()
		for i in [1, 3, 5, 6, 7, 8, 9, 13]:
			df_adult_income.iloc[:, i] = label_encoder.fit_transform(df_adult_income.iloc[:, i])

		X_train = df_adult_income.iloc[0:19034, :14]
		y_train = df_adult_income.iloc[0:19034, 14]
		X_val = df_adult_income.iloc[19035:24130, :14]
		y_val = df_adult_income.iloc[19035:24130, 14]
		X_test = df_adult_income.iloc[24131:, :14]
		y_test = df_adult_income.iloc[24131:, 14]


		joined_trained = X_train.join(y_train)
		joined_trained = joined_trained.values.tolist()
		decisionTree = growDecisionTreeFrom(joined_trained, evaluationFunction=gini)
